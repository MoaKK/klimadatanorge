  ...
  return (
    <div className="absolute bottom-8 left-1/2 z-10 w-[clamp(16rem,50vw,36rem)] -translate-x-1/2 rounded-xl bg-background/80 px-6 py-4 backdrop-blur-sm animate-in fade-in duration-1000">
      ...
      <div className="mb-3 flex items-center justify-between">
        <span className="text-xs text-muted-foreground sm:text-sm">{PERIODS[0].label}</span>
        ...
        <span className="text-xs text-muted-foreground sm:text-sm">{PERIODS[PERIODS.length - 1].label}</span>
      </div>
      <Slider
        min={0}
